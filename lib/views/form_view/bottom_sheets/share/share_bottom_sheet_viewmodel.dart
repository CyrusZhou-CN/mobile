...
class ShareBottomSheetViewModel extends BaseViewModel {
  ...
  var shareWithUsers = [];
  var permissionLevels = ["Can Read", "Can Write", "Can Share", "Full Access"];
  ...
  addShare({
    ...
    for (var user in users) {
      var permissions = {"read": 1, "user": user["value"]};
      ...
      var response = await locator<Api>().shareAdd(doctype, name, permissions);
  ...
  updatePermission({
    ...
    if (currentPermission == "Can Write") {
      reqs.add({"permission_to": "write", "value": 0});
    } else if (currentPermission == "Can Share") {
      reqs.add({"permission_to": "share", "value": 0});
    } else if (currentPermission == "Full Access") {
      reqs.addAll([
        {"permission_to": "share", "value": 0},
        {"permission_to": "write", "value": 0},
      ]);
    }

    if (newPermission == "Can Write") {
      reqs.add({"permission_to": "write", "value": 1});
    } else if (newPermission == "Can Share") {
      reqs.add({"permission_to": "share", "value": 1});
    } else if (newPermission == "Full Access") {
      reqs.addAll([
        {"permission_to": "share", "value": 1},
        {"permission_to": "write", "value": 1},
      ]);
    }
